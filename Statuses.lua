...
--		duration = 5,
	...
	cantusetwice = {
		...
		onDisableMove = function(pokemon)
			for _, move in pairs(pokemon.moveset) do
				...
				pokemon:disableMove(move.id, false, nil)
				end
			end
		end,
	},
	...
	commanded = {
		...
		noCopy = true,
		onStart = function(pokemon)
			self:boost({atk = 2, spa = 2, spe = 2, def = 2, spd = 2}, pokemon)
		end,
		onDragOutPriority = 2,
		onDragOut = function()
			return false
		end,
		onModifyPokemonPriority = -11,
		onModifyPokemon = function(pokemon)
	...
	maxraid = {
		...
		onDamage = function(damage, target, source, effect)
			if self.effectData.hasShield and self.effectData.hasShield > 0 then
				...
				damage = math.floor(damage/3) --When shield up do no damage
			end
			if self.effectData.dmaxStages > 1 and (target.hp - damage > (math.floor(target.maxhp/3) * self.effectData.dmaxStages-1)) then
			...
			if damage >= math.floor(target.maxhp/3) then
				return math.floor(target.maxhp/3)
			end
		end,
		onDragOutPriority = 2,
		onDragOut = function(pokemon)
