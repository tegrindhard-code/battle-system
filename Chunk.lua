return function(_p)
	...
	function Chunk:enterDoor(door)
		...
		if door.id == "C_chunk89:b" or door.id == "C_chunk89:a" then
			_p.Events["onBeforeEnter_C_chunk89:a"] = function()
				local choose = _p.NPCChat:say('[y/n]Leave the safari zone early?')
				if choose then 
					_p.WalkEvents:removeSafariStepUI()
					return false
				else
					local d = _p.DataManager.currentChunk:getDoor('C_chunk89:a')
			...
			_p.Events["onBeforeEnter_C_chunk89:b"] = function()
				local choose = _p.NPCChat:say('[y/n]Leave the safari zone early?')
				if choose then 
					_p.WalkEvents:removeSafariStepUI()
					return false
				else
					local d = _p.DataManager.currentChunk:getDoor('C_chunk89:b')
					if d and _p.player.Character then
						local bp = d.Position - d.CFrame.lookVector * 3
						_p.MasterControl:WalkTo(bp)
					end
