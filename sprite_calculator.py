#!/usr/bin/env python3
"""
Sprite Size Calculator for 3D Battle System
...
Note: For the Blender model converter, use textureconverter.py
"""
...
class SpriteScaleCalculator:
    """Calculates sprite sizes and 3D model scaling parameters."""
    ...
    def generate_scale_guide(self, sprite_width: int, sprite_height: int) -> Dict:
        ...
        return {
            ...
            "recommended_3d_scales": {
                "small_pokemon": self.calculate_3d_scale(sprite_scale=0.75),
                "medium_pokemon": self.calculate_3d_scale(),
                "large_pokemon": self.calculate_3d_scale(sprite_scale=1.5),
...
def main():
    """CLI interface for sprite scale calculator."""
    calc = SpriteScaleCalculator()

    if len(sys.argv) < 2:
        print("Sprite Scale Calculator - 3D Battle System")
        print("=" * 50)
        print("\nFor the Blender MODEL CONVERTER, use: textureconverter.py")
        print("\nThis tool calculates sprite sizes and scale factors:")
        print("\nUsage:")
        print("  python sprite_calculator.py <width> <height> [sprite_scale]")
        print("  python sprite_calculator.py guide <width> <height>")
        print("\nExamples:")
        print("  python sprite_calculator.py 96 96")
        print("  python sprite_calculator.py 120 120 1.2")
        print("  python sprite_calculator.py guide 96 96")
        return
    ...
    if sys.argv[1] == "guide":
        ...
        guide = calc.generate_scale_guide(width, height)
        ...
        width_studs, height_studs = calc.calculate_sprite_size(width, height, sprite_scale)
        model_scale = calc.calculate_3d_scale(sprite_scale)

        print(f"\n2D Sprite: {width}x{height} pixels")
        print(f"  → {width_studs:.2f} x {height_studs:.2f} studs")
        print(f"\n3D Model Scale: {model_scale:.3f}")
        print(f"  (Base: {calc.BASE_MODEL_SCALE} × Sprite: {sprite_scale})")

        print("\nSpecial Variants:")
        print(f"  Alpha:   {calc.calculate_3d_scale(sprite_scale, is_alpha=True):.3f}")
        print(f"  Totem:   {calc.calculate_3d_scale(sprite_scale, is_totem=True):.3f}")
        print(f"  Dynamax: {calc.calculate_3d_scale(sprite_scale, dynamax_level=1):.3f}")
