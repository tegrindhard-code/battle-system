	...
	for n, a in pairs(request.active) do
		...
		if self.name ~= '#Wild' then
			local s, r = pcall(function()
				...
				-- advanced ai logic
				if not move then
					-- Check if this is a smart trainer (based on difficulty/trainer data)
					local isSmartTrainer = self:getDifficulty() >= 3 or (battle.npcTrainerData and battle.npcTrainerData.Smart)
					...
					local setupMoves = {
						...
						"shiftgear", "irondefense", "victorydance", "noretreat", "courtchange",
						"tidyup"
					}
					local contraryMoves = {
					...
					local boostClearMoves = {
						"haze", "clearsmog", "chillingwater"
					}
					local phazingMoves = {
						"whirlwind", "roar", "dragontail", "circlethrow"
					}

					-- function to compute est dmg
					local function getEstimatedDamage(m)
						local moveData = battle:getMoveCopy(pokemon.moves[m])
						moveData = battle:runEvent('ModifyMove', pokemon, target, moveData, moveData)
						...
						if moveData.flags.charge and not pokemon:hasItem('powerherb') and not (
					...
					-- 5A. Boost Clear / Phazing clause - if opponent has boosted stats
					if not move and target:positiveBoosts() >= 2 then
						-- Try boost-clearing moves
						for _, v in pairs(pokemon:getEnabledMoves()) do
							local moveData = battle:getMoveCopy(pokemon.moves[v])
							if table.find(boostClearMoves, moveData.id) then
								move = v
								break
							end
						end
						-- Try phazing moves if no boost-clearing moves
						if not move then
							for _, v in pairs(pokemon:getEnabledMoves()) do
								local moveData = battle:getMoveCopy(pokemon.moves[v])
								if table.find(phazingMoves, moveData.id) and not target:hasAbility('suctioncups') then
									move = v
									break
								end
							end
						end
						-- Try self-switch moves as last resort
						if not move and self.pokemonLeft > 1 then
							for _, v in pairs(pokemon:getEnabledMoves()) do
								local moveData = battle:getMoveCopy(pokemon.moves[v])
								if moveData.selfSwitch and not pokemon:pranksterCheck(moveData, target) then
									move = v
									break
								end
							end
					...
					-- 2B. Outspeed KO/2HKO clause
					if not move then
						local bestKOMove, best2HKOMove = nil, nil
						for _, v in pairs(pokemon:getEnabledMoves()) do
							local moveData, damage = getEstimatedDamage(v)
							damage = tonumber(damage) or 0
							local canOutspeed = pokemon:getStat('spe') > target:getStat('spe')
							if battle:getPseudoWeather('trickroom') then
								canOutspeed = not canOutspeed
							end

							if canOutspeed then
								-- Check for immediate KO
								if damage >= target.hp and not bestKOMove then
									bestKOMove = v
								end
								-- Check for 2HKO potential (if we can outspeed twice)
								if damage * 2 >= target.hp and damage > 0 and not best2HKOMove then
									best2HKOMove = v
								end
							end
						end
						-- Prioritize KO over 2HKO
						move = bestKOMove or best2HKOMove
					end

					-- 3. Hard Switch (66% HP threshold for smart trainers, 50% for others)
					local switchThreshold = isSmartTrainer and 0.66 or 0.50
					if not move and target.lastMove ~= '' and self.pokemonLeft > 1 and pokemon.hp <= pokemon.maxhp * switchThreshold and not pokemon.trapped and pokemon.activeTurns > 1 and pokemon:positiveBoosts() < 2 then
					...
					-- 5B. Setup clause (first turn only, HP > 66% threshold)
					if not move and pokemon.activeTurns <= 1 then
						-- Smart trainers check HP threshold, others always try setup
						local canSetup = (not isSmartTrainer) or (pokemon.hp > pokemon.maxhp * 0.66)
						if canSetup then
							for _, v in pairs(pokemon:getEnabledMoves()) do
								local moveData = battle:getMoveCopy(pokemon.moves[v])
					...
					-- 6. Status clause (first 2 turns target is active, HP > 66% threshold)
					if not move and target.activeTurns <= 2 and target.status == '' and not (pokemon:hasAbility('prankster') and target:hasType('Dark')) then
						-- Smart trainers check HP threshold, others always try status
						local canStatus = (not isSmartTrainer) or (pokemon.hp > pokemon.maxhp * 0.66)
						if canStatus then
							for _, v in pairs(pokemon:getEnabledMoves()) do
								local moveData = battle:getMoveCopy(pokemon.moves[v])
					...
					if not move then
						...
						for _, v in pairs(pokemon:getEnabledMoves()) do
							local moveData, damage = getEstimatedDamage(v)
							-- Ensure damage is a number (handle edge cases where getDamage returns a table)
							damage = tonumber(damage) or 0
							estDamage[v] = damage
							if damage > 0 then
...
-- AI Switch Selection - intelligently chooses which Pokemon to switch to
function BattleSide:AIChooseSwitch(choices, opponent)
	-- Find the best Pokemon to switch to based on type matchups, HP, and movesets
	local battle = self.battle
	local bestScore = -9999
	local bestSwitch = nil

	for i, p in pairs(self.pokemon) do
		if not p.isActive and not p.fainted and p.hp > 0 then
			local score = 0

			-- Base score on current HP percentage
			score = score + (p.hp / p.maxhp) * 30

			-- Evaluate type matchups against opponent
			if opponent then
				local effectiveness = 1
				for _, oppType in pairs(opponent:getTypes()) do
					for _, myType in pairs(p:getTypes()) do
						-- Check how our type resists opponent's types
						local resist = battle.data.TypeChart[myType][oppType] or 1
						if resist < 1 then
							score = score + 20 -- We resist opponent's STAB
						elseif resist > 1 then
							score = score - 15 -- We're weak to opponent's STAB
						end
					end
				end

				-- Check if we have super-effective moves against opponent
				for _, move in pairs(p.moveset) do
					if move and move.id then
						local moveData = battle.data.Movedex[move.id]
						if moveData and moveData.basePower and moveData.basePower > 0 then
							local moveEff = 1
							for _, oppType in pairs(opponent:getTypes()) do
								moveEff = moveEff * (battle.data.TypeChart[oppType][moveData.type] or 1)
							end
							if moveEff > 1 then
								score = score + (moveEff * 25) -- Bonus for super-effective moves
							elseif moveEff < 1 then
								score = score - 10 -- Penalty for resisted moves
							end
						end
					end
				end
			...
			-- Prefer Pokemon with higher stats
			score = score + (p.stats.spe / 10) -- Bonus for speed
			score = score + ((p.stats.atk + p.stats.spa) / 20) -- Bonus for offensive stats

			-- Update best switch if this Pokemon scores higher
	...
	-- If we found a valid switch, use it
	if bestSwitch then
		for n, _ in pairs(self.switchQueue) do
			choices[n] = 'switch ' .. bestSwitch
		end
		return true
	end
