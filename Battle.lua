return function(_p)
	...
	local BattleClient = Utilities.class({
		...
		-- Define camera positions for dynamic battle camera
		self.cameraSides = {
			-- Default/neutral camera position
			neutral = {
				FieldOfView = 35,
				CoordinateFrame = CFrame.new(-5.59, 4.56, -8.91, -.746, .063, -.663, 0, .996, .094, .666, .07, -.742),
			},
			-- Camera looking at user's side (opponent is attacking)
			p1 = {
				FieldOfView = 35,
				CoordinateFrame = CFrame.new(-3.5, 5.5, -12.0, -.85, .1, -.5, 0, .98, .2, .52, .17, -.83),
			},
			-- Camera looking at opponent's side (user is attacking)
			p2 = {
				FieldOfView = 35,
				CoordinateFrame = CFrame.new(-7.5, 5.5, -6.5, -.65, .05, -.75, 0, .998, .06, .76, .04, -.65),
			},
		}

		-- Track current camera target for smooth transitions
		self.currentCameraTarget = "neutral"
		...
		if self.gameType == 'doubles' then
			...
			-- Update camera sides for doubles battles
			self.cameraSides.neutral.FieldOfView = 40
			self.cameraSides.neutral.CoordinateFrame = CFrame.new(-2.32, 6.873, -11.056, -.868, .162, -.469, 0, .945, .326, .497, .283, -.821)
			self.cameraSides.p1.FieldOfView = 40
			self.cameraSides.p1.CoordinateFrame = CFrame.new(-1.0, 7.0, -13.5, -.88, .15, -.45, 0, .95, .32, .48, .28, -.84)
			self.cameraSides.p2.FieldOfView = 40
			self.cameraSides.p2.CoordinateFrame = CFrame.new(-4.0, 7.0, -9.0, -.85, .08, -.52, 0, .99, .15, .53, .13, -.84)
		end
		...
		if self.battleSceneType == 'LabDouble' then
			...
			self.cameraSides.neutral.CoordinateFrame = CFrame.new(-3.32, 6.873, -12.056, -.868, .162, -.469, 0, .945, .326, .497, .283, -.821)
		end
...
--]]
	...
	-- Trainer pointing animation system
	function BattleClient:playTrainerPointAnimation(pokemonId)
		if self.fastForward then return end
		if not pokemonId then return end

		-- Determine which trainer to animate based on pokemon side
		local trainerObj = nil
		if pokemonId:sub(1, 2) == "p1" then
			-- Player's Pok�mon - animate player
			trainerObj = self.playerModelObj
		elseif pokemonId:sub(1, 2) == "p2" then
			-- Opponent's Pok�mon - animate opponent
			trainerObj = self.trainerModelObj
		end

		if trainerObj and trainerObj.PointAnimation then
			-- Stop any currently playing point animation
			if trainerObj.PointAnimation.IsPlaying then
				trainerObj.PointAnimation:Stop()
			end

			-- Lock trainer in place by storing original position
			local originalCFrame = trainerObj.Root.CFrame
			local originalPosition = trainerObj.BodyPosition.Position

			-- Increase BodyPosition force to prevent animation root motion from moving trainer
			local originalMaxForce = trainerObj.BodyPosition.MaxForce
			trainerObj.BodyPosition.MaxForce = Vector3.new(math.huge, math.huge, math.huge)

			-- Play point animation
			trainerObj.PointAnimation:Play()

			-- Keep trainer locked in place during animation
			local lockConnection
			lockConnection = game:GetService('RunService').RenderStepped:Connect(function()
				if trainerObj.Root and trainerObj.BodyPosition then
					trainerObj.Root.CFrame = originalCFrame
					trainerObj.BodyPosition.Position = originalPosition
				end
			end)

			-- Auto-stop after animation duration and restore position locking
			spawn(function()
				wait(1.0)
				if trainerObj.PointAnimation.IsPlaying then
					trainerObj.PointAnimation:Stop()
				...
				-- Disconnect position lock and restore original MaxForce
				if lockConnection then
					lockConnection:Disconnect()
				end
				if trainerObj.BodyPosition then
					trainerObj.BodyPosition.MaxForce = originalMaxForce
				end
			end)
		end
	end

	-- Dynamic camera system for battle
	function BattleClient:setCameraToSide(side, duration)
		if self.cameraTransitioning then return end
		duration = duration or 0.4

		local targetCamera = self.cameraSides[side]
		if not targetCamera then return end

		self.currentCameraTarget = side
		self.cameraTransitioning = true

		local cam = workspace.CurrentCamera
		local startCFrame = cam.CFrame
		local startFOV = cam.FieldOfView

		local targetCFrame = targetCamera.CoordinateFrame + self.sceneOffset
		local targetFOV = targetCamera.FieldOfView

		local elapsed = 0
		local connection
		connection = stepped:Connect(function(dt)
			elapsed = elapsed + dt
			...
			-- Lerp camera CFrame
			cam.CFrame = startCFrame:Lerp(targetCFrame, smoothAlpha)
			-- Lerp FOV
			cam.FieldOfView = startFOV + (targetFOV - startFOV) * smoothAlpha

			if alpha >= 1 then
				connection:Disconnect()
				self.cameraTransitioning = false

				-- Update battleCamera to reflect new position
				self.battleCamera.CoordinateFrame = targetCamera.CoordinateFrame
				self.battleCamera.FieldOfView = targetCamera.FieldOfView
			end
		end)
	end

	function BattleClient:getCameraSideForPokemon(pokemonId)
		-- Determine which side the camera should look at based on the attacker
		-- If p1 is attacking, show p2's side (opponent's side)
		-- If p2 is attacking, show p1's side (user's side)
		if not pokemonId then return "neutral" end

		-- Parse pokemon ID to determine side
		if pokemonId:sub(1, 2) == "p1" then
			return "p2"  -- p1 attacking, show opponent side
		elseif pokemonId:sub(1, 2) == "p2" then
			return "p1"  -- p2 attacking, show user side
		end
	...
	function BattleClient:setupScene()
		...
		-- Position player closer (2 studs forward from their base position)
		local pcf = self.CoordinateFrame1 * CFrame.new(0, 3*scale, 2)
		...
		if self.kind == 'spectate' then -- 2v2specdo: spectating 2v2s O.o
			...
			-- Load pointing animation for player (check if R15 or R6)
			local isR15 = pObj.Humanoid.RigType == Enum.HumanoidRigType.R15
			pObj.PointAnimation = pObj.Humanoid:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId[isR15 and 'R15_Point' or 'R6_Point'] })

			self.playerModel = pObj.Model
			self.playerModelObj = pObj
		end
		if self.kind == 'trainer' then
			-- Position trainer closer (2 studs forward from their base position)
			local trainerCFrame = self.CoordinateFrame2 * CFrame.new(0, 3*scale, 2)
			local tObj = getScaledCharacter(self.trainerModel or _p.storage.Models.NPCs[self.trainer.ModelName or self.trainer.TrainerClass], trainerCFrame)
			tObj.WalkAnimation = tObj.Humanoid:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.NPCWalk })

			-- Load pointing animation for NPC trainers
			tObj.PointAnimation = tObj.Humanoid:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.NPCPoint })

			self.trainerModel = tObj.Model
			self.trainerModelObj = tObj
		elseif self.kind == 'pvp' then
			-- Position opponent closer (2 studs forward)
			local opponentCFrame = self.CoordinateFrame2 * CFrame.new(0, 3*scale, 2)
			local tObj = getScaledCharacter(self.opponent.Character, opponentCFrame)

			-- Load pointing animation (check if R15 or R6)
			local isR15 = tObj.Humanoid.RigType == Enum.HumanoidRigType.R15
			tObj.PointAnimation = tObj.Humanoid:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId[isR15 and 'R15_Point' or 'R6_Point'] })
