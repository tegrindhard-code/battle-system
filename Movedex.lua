...
-- current 951 moves
-- current save structure previously supported 1023, now supports 2047 thanks to Infrared :)
-- Infrared also basically did all of Gen 9 for BL/RC
...
punch: Power is multiplied by 1.3 when used by a Pokemon with the Ability Iron Fist.
...
wind: Has no effect on Pokemon with the Ability Wind Rider.
--]]
...
return {
	...
	['upperhand'] = {
		...
		onTry = function(source, target)
			...
			if not decision or decision.choice ~= 'move' or (decision.move.category == 'Status' and decision.move.id ~= 'mefirst') or target.volatiles.mustrecharge or decision.move.priority < 0.1 then
	...
	['dragoncheer'] = {
		...
		effect = {
			onStart = function(target, source, effect)
				if target.volatiles['focusenergy'] then
					return false
				end

				local silence = false
				local validEffects = {"costar", "imposter", "psychup", "transform"}
				for eff in ipairs(validEffects) do
					if effect and effect.id == eff then
				...
				self.effectData.isDragon = target:hasType('Dragon')
			end,
			onModifyCritRatio = function(critRatio, source)
				return critRatio + (self.effectData.isDragon and 2 or 1)
			end,
		},
		target = "adjacentAlly",
		type = "Dragon"
	...
	['hardpress'] = {
		...
		basePowerCallback = function(pokemon, target)
			...
			local bp = math.floor(math.floor((100 * (100 * math.floor(hp * 4096 / maxHP)) + 2048 - 1) / 4096) / 100) or 1
			self:debug("BP for Hard Press: " .. bp .. ", with hp of " .. hp .. " over max HP of " .. maxHP)	
			return bp	
		end,
		category = "Physical",
		id = "hardpress",
	...
	['tachyoncutter'] = {
		...
		pp = 10,
		multihit = 2,
		flags = {protect = true, mirror = true, slicing = true},
		target = "normal",
		type = "Steel"
	},
	['mightycleave'] = {
	...
	['terastarstorm'] = {
		...
		onModifyMove = function(move, pokemon)
			local forme = pokemon.template.forme
			if forme == 'Stellar' then
	...
	['electroshot'] = {
		...
		onTry = function(attacker, defender, move)
			...
			if self:isWeather({'raindance', 'primordialsea'}) or Not(self:runEvent('ChargeMove', attacker, defender, move)) then
	...
	['ivycudgel'] = {
		...
		onModifyMove = function(move, pokemon)
			local forme = pokemon.template.forme
			if forme == 'Wellspring' then
				move.type = 'Water'
			elseif forme == 'Hearthflame' then
				move.type = 'Fire'
			elseif forme == 'Cornerstone' then
				move.type = 'Rock'
			end
		end,
		target = "normal",
	...
	['syrupbomb'] = {
		...
		name = "Syrup Bomb",
		pp = 10,
		flags = {protect = true, mirror = true, bullet = true},
		effect = {
			noCopy = true,
			duration = 4,
			...
			onResidual = function(pokemon)
				self:boost({spe = -1}, pokemon, self.effectData.source)
			end,
	...
	['matchagotcha'] = {
		...
		name = "Matcha Gotcha",
		pp = 15,
		flags = {protect = true, mirror = true, defrost = true, heal = true},
		drain = {1, 2},
		secondary = {
			chance = 20,
			status = 'brn'
	...
	['comeuppance'] = {
		...
		damageCallback = function(pokemon)
			if not pokemon.volatiles['comeuppance'] then return 0 end
			return math.max(pokemon.volatiles['comeuppance'].damage or 1, 1)
		end,
		category = "Physical",
		id = "comeuppance",
		name = "Comeuppance",
		...
		onTryHit = function(target, source, move)
			if not source.volatiles['comeuppance'] then return false end
			if Not(source.volatiles['comeuppance'].position) then return false end
		end,
		effect = {
			duration = 1,
			noCopy = true,
	...
	['doubleshock'] = {
		...
		flags = {protect = true, mirror = true, contact = true},
		onTryHit = function(target, source, move)
			if not source:hasType('Electric') then return false end
		end,
		self = {
			onHit = function(pokemon)
				local otherType = '???'
	...
	['bitterblade'] = {
		...
		name = "Bitter Blade",
		pp = 10,
		flags = {protect = true, mirror = true, heal = true, contact = true, slicing = true},
		drain = {1, 2},
		target = "normal",
		type = "Fire"
	},
	...
	['ragefist'] = {
		...
		basePowerCallback = function(pokemon)
			local basePower = math.min(350, 50 + 50 * pokemon.timesAttacked)
			local debugMessage = string.format("timesAttacked: %d, basePower: %d", pokemon.timesAttacked, basePower)
			--self:debug(debugMessage)
			return basePower
		end,
		category = "Physical",
		id = "ragefist",
		...
		onHit = function(source, target)
			...
			for _, side in pairs(self.sides) do
				for _, active in pairs(side.active) do
					if active:removeVolatile('substitute') then
						success = true
					end
				end 
			end
			local removeAll = {spikes=true, toxicspikes=true, stealthrock=true, stickyweb=true}
			for sc in pairs(removeAll) do
				if target.side:removeSideCondition(sc) then
					if removeAll[sc] then
						self:add('-sideend', target.side, self:getEffect(sc).name, '[from] move: Tidy Up', '[of] ' .. target)
					end
				end
			end
			for sc in pairs(removeAll) do
				if source.side:removeSideCondition(sc) then
					self:add('-sideend', source.side, self:getEffect(sc).name, '[from] move: Tidy Up', '[of] ' .. source)
				end
			end
			if success then
				self:add('-activate', source, 'move: Tidy Up')
			end
			return (self:boost({atk = 1, spe = 1}, source, source, nil, false, true) and true) or success
		end,
		target = "self",
		type = "Normal"
	},
	['shedtail'] = {
		...
		onTryHit = function(target)
			if target.volatiles['substitute'] then
				self:add('-fail', target, 'move = Shed Tail')
				return null
			end
			if target.hp <= target.maxhp/2 or target.maxhp == 1 then -- Shedinja clause
				self:add('-fail', target, 'move = Shed Tail', '[weak]')
				return null
			end
		end,
		...
		effect = {
			onStart = function(target, effect)
				if effect and effect.id == 'shedtail' then
					self:add('-start', target, 'Substitute', '[from] move: Shed Tail')
				else
					self:add('-start', target, 'Substitute')
				end
				self.effectData.hp = math.floor(target.maxhp / 2)
	...
	['electrodrift'] = {
		...
		onBasePower = function(basePower, source, target, move)
			if move.typeMod and move.typeMod > 0 then
	...
	['collisioncourse'] = {
		...
		onBasePower = function(basePower, source, target, move)
			if move.typeMod and move.typeMod > 0 then
				...
				return self:chainModify({5461, 4096}) -- Buff base power by 5461/4096
			end
		end,
		target = "normal",
		type = "Fighting",
	...
	['psyblade'] = {
		...
		id = 'psyblade',
		pp = 15,
		flags = {protect = true, mirror = true, contact = true, slicing = true},
		onBasePower = function(basePower, pokemon, target)
	...
	['ragingbull'] = {
		...
		onTryHit = function(pokemon)
			...
			if pokemon:runImmunity('Fighting') then
	...
	['kowtowcleave'] = {
		...
		name = "Kowtow Cleave",
		pp = 10,
		flags = {contact = true, protect = true, mirror = true, slicing = true},
		target = "normal",
		type = "Dark"
	},
	['filletaway'] = {
		...
		id = "filletaway",
		pp = 10,
		flags = {snatch = true, sound = true},
		onTry = function(source)
			if (source.hp <= (source.maxhp * 50 / 100) or source.maxhp == 1) then return false end
		end,
		onTryHit = function(pokemon, target, move)
			if not (self:boost(move.boosts)) then return end --check
		end,
		onHit = function(pokemon)
			self:directDamage(pokemon.maxhp * 50 / 100)
		end,
	...
	['doodle'] = {
		...
		flags = {},
		onTryHit = function(pokemon)
			local bannedAbilities = {multitype=true,stancechange=true,asone=true,battlebond=true,comatose=true,commander=true,disguise=true,
				gulpmissle=true,hadronengine=true,orichalcumpulse=true,iceface=true,powerconstruct=true,protosynthesis=true,quarkdrive=true,
				rkssystem=true,schooling=true,shieldsdown=true,zenmode=true,zerotohero=true}
			if bannedAbilities[pokemon.ability] then return false end
		end,
		onHit = function(target, source, move)
			local success = false
			for _, pokemon in ipairs(source.side.pokemon) do
				if pokemon.ability == target.ability then
					continue
				end
				local oldAbility = pokemon:setAbility(target.ability)
				if oldAbility then
					self:add('-ability', pokemon, target.ability, '[from] move: Doodle')
					success = true
				elseif not success and oldAbility == nil then
					success = nil
				end
				continue
			end
			...
			onHit = function(pokemon)
				...
				for _, sc in pairs(sideConditions) do
					if pokemon.hp > 0 and pokemon.side:removeSideCondition(sc) then
						self:add('-sideend', pokemon.side, self:getEffect(sc).name, '[from] move = Mortal Spin', '[of] ' .. pokemon)
					end
				end
	...
	['saltcure'] = {
		...
		effect = {
			...
			onResidualOrder = 13,
			onResidual = function(pokemon)
				self:damage(pokemon.baseMaxhp / (pokemon:hasType('Water', 'Steel') and 4 or 8))
			end,
			onEnd = function(pokemon)
	...
	['orderup'] = {
		...
		onAfterMoveSecondarySelf = function(pokemon, target, move)
			if not pokemon.volatiles['commanded'] then
				return
			end

			local tatsugiri = pokemon.volatiles['commanded'].source
			if tatsugiri.baseSpecies ~= 'Tatsugiri' then
				return -- Should never happen
			end

			local forme = tatsugiri.baseSpecies.forme
			if forme == 'Droopy' then
				self.boost({def = 1}, pokemon, pokemon)
			elseif forme == 'Stretchy' then
				self.boost({spe = 1}, pokemon, pokemon)
			else
	...
	['axekick'] = {
		...
		hasCustomRecoil = true,
		onMoveFail = function(target, source, move)
			self:damage(source.maxhp / 2, source, source, 'axekick')
		end,
		secondary = {
			chance = 30,
			volatileStatus = 'confusion'
	...
	['silktrap'] = {
		...
		effect = {
			...
			onTryHit = function(target, source, move)
				...
				if move.flags['contact'] then
					self:boost({spe=-1}, source, target, self:getMove('silktrap'))
				end
				return null
			end
		},
	...
	['ceaselessedge'] = {
		...
		effect = {
			onStart = function(side)
				self:add('-sidestart', side, 'move = Spikes')
			end,
		},	
		secondary = {},
		target = "normal",
	...
	['aquastep'] = {
		...
		name = "Aqua Step",
		pp = 10,
		flags = {contact = true, protect = true, mirror = true, dance = true},
		secondary = {
			chance = 100,
			self = {
				boosts = {
		...
		onTryHit = function(source, target)
			...
			for _, pokemon in ipairs(source.side.pokemon) do
				if pokemon.fainted then
					table.insert(faintedPokemon, pokemon)
				end
			end
			if #faintedPokemon == 0 then
				return false -- No fainted Pok�mon on the user's side
			end
			local revivedPokemon = faintedPokemon[1] -- Revive the first fainted Pok�mon
			revivedPokemon.hp = math.floor(revivedPokemon.maxhp / 2)
			revivedPokemon.status = ''
			revivedPokemon.fainted = false
			revivedPokemon.faintQueued = false
			revivedPokemon.side.pokemonLeft = revivedPokemon.side.pokemonLeft + 1
			self:add('-heal', revivedPokemon, math.floor(revivedPokemon.maxhp / 2), '[from] move: Revival Blessing')
		end,
	...
	['octolock'] = {
		...
		condition = {
			...
			onResidual = function(pokemon)
				...
				self:boost({def = -1, spd = -1}, pokemon, source, self:getMoveCopy('octolock'))
			end,
	...
	['boltbeak'] = {
		...
		basePowerCallback = function(pokemon, target, move)
			...
			self:debug("Bolt beak unboosted")
	...
	['fishiousrend'] = {
		...
		basePowerCallback = function(pokemon, target, move)
			...
			self:debug("Fishious Rend unboosted")
			return move.basePower
		end,
		category = 'Physical',
		name = 'Fishious Rend',
	...
	['courtchange'] = {
		...
		onHitField = function(target, source)
			...
			local success = false
			-- Reworked by Infrared, the original was horrid like the other Gen 8 PBB garbage
			local sourceSC = source.side.sideConditions
			local targetSC = source.side.foe.sideConditions
			local sourceTemp = {}
			local targetTemp = {}

			for id in pairs(sourceSC) do
				if not table.find(sideConditions, id) then continue end
				sourceTemp[id] = sourceSC[id]
				sourceSC[id] = nil
				success = true
			end

			for id in pairs(targetSC) do
				if not table.find(sideConditions, id) then continue end
				targetTemp[id] = targetSC[id]
				targetSC[id] = nil
				success = true
			end

			for id in pairs(sourceTemp) do
				targetSC[id] = sourceTemp[id]
			end

			for id in pairs(targetTemp) do
				sourceSC[id] = targetTemp[id]
			end
	...
	['gmaxstunshock'] = {
		...
		isMax = "Toxtricity",	
		priority = 0,
		onHit = function(source,targetSide) 
			if targetSide.foe.active then
				local i = math.random(2) 
				if i == 1 then
					self:trySetStatus('par', source) 
				else 
					self:trySetStatus('psn', source) 
				end
			end
		end,	
		target = "adjacentFoes",
	...
	['gmaxstonesurge'] = {
		...
		effect = {
			...
			onSwitchIn = function(pokemon)
				...
				elseif pokemon:hasAbility('tangledfeet') then
					local mult = false
					local factor = false 
					local damage =  false
					print ("User has tangled feet")
	...
	['gmaxresonance'] = {
		...
		effect = {
			...
			onStart = function(side)
				self:add('-sidestart', side, 'move = Aurora Veil', side:getSCDur('auroraveil'))
			end,
			onResidualOrder = 21,
	...
	['gmaxvoltcrash'] = {
		...
		isMax = "Pikachu",
		onHit = function(source) 
			if source.side.foe.active then
				self:trySetStatus('par', source) 
			end
		end,		
		target = "adjacentFoes",
	...
	['gmaxcannonade'] = {
		...
		onResidual = function(targetSide,pokemon) 
			if targetSide.active then
				if pokemon.type == 'Water' then self:damage(pokemon.baseMaxhp / 6, pokemon)
				end
			end
		end,
		onEnd = function(targetSide,pokemon) 
			if targetSide.active then
				if pokemon.Type == 'Water' then self:damage(pokemon.baseMaxhp / 6, pokemon)
				end
	...
	['gmaxcentiferno'] = {
		...
		priority = 0,
		isMax = "Centiskorch",
		onHit = function(source,move,pokemon) 
			if source.side.foe.active then
				pokemon:addVolatile('partiallytrapped', source, 'move = G-Max Centiferno')
			end
		end,
		target = "adjacentFoes",
		type = "Fire",
	...
	gmaxfoamburst = {
		...
		isMax = "Kingler",
		onHit = function(source, target) 
			for _, side in pairs(target.side.foe.active) do
				self:boost({spe = -2}, side)
			end
		end,
		target = "adjacentFoe",
		type = "Water",
	...
	['gmaxwildfire'] = {
		...
		onResidual = function(targetSide, pokemon) 
			if targetSide.active then
				if pokemon.type == ('Fire') then self:damage(pokemon.baseMaxhp / 6, pokemon)
				end
			end   
		end,
		onEnd = function(targetSide, pokemon) 
	...
	maxstarfall = {
		...
		isMax = true,
		onHit = function(source)  
			if not (source.volatiles['dynamax']) then return end
	...
	['maxguard'] = {
		...
		effect = {
			duration = 1,
			onStart = function(target)
				self:add('-singleturn', target, 'move = Max Guard')
			end,
			onTryHitPriority = 3,
			onTryHit = function(target, source, move)
				local bypassMaxGuard = {'acupressure', 'afteryou', 'allyswitch', 'aromatherapy', 'aromaticmist', 'coaching', 'confide', 'copycat', 'curse', 'decorate', 'doomdesire', 'feint', 'futuresight', 'gmaxoneblow', 'gmaxrapidflow', 'healbell', 'holdhands', 'howl', 'junglehealing', 'lifedew', 'meanlook', 'perishsong', 'playnice', 'powertrick', 'roar', 'roleplay', 'tearfullook'}
				if table.find(bypassMaxGuard, move.id) then
					return
				end
				self:add('-activate', target, 'move = Max Guard', source)
				local lockedmove = source:getVolatile('lockedmove')
				if lockedmove then
					-- Outrage counter is reset
					if source.volatiles['lockedmove'].duration == 2 then
	...
	['tripleaxel'] = {
		...
		basePower = 20,
		basePowerCallback = function(pokemon)
			pokemon:addVolatile('tripleaxel')
			return 20 * pokemon.volatiles['tripleaxel'].hit
		end,
		category = "Physical",
		name = "Triple Axel",
		id = 'tripleaxel',
		...
		onAfterMove = function(pokemon)
			pokemon:removeVolatile('tripleaxel')
		end,
		target = "normal",
		type = "Ice",
	},
	['junglehealing'] = {
		...
		flags = {heal = true, authentic=true, mystery=true},
		onHit = function(pokemon)
			local success = not self:heal(self:modify(pokemon.maxhp, .25))
			return pokemon:cureStatus() or success
		end,
	...
	['psychicterrain'] = {
		...
		effect = {
			...
			onTryHit = function(target, source, effect)
				if not target:isGrounded() or target:isSemiInvulnerable() or target:isAlly(source) then return end
			...
			onStart = function(battle, source, effect)
				self:add('-terrainstart', self:getTerrainDuration())
			...
			onResidualOrder = 21,
			onResidual = function()
				local dur = self:getTerrainDuration()
				local durationMessage = tostring(dur) .. " turn(s) remaining."
				self:add('-terrain', 'Psychic Terrain', durationMessage, dur)
			end,
	...
	['revelationdance'] = {
		...
		pp = 15,
		flags = {protect = true, mirror = true, dance = true},
		onModifyMove = function(move, pokemon)
	...
	['solarblade'] = {
		...
		name = "Solar Blade",
		pp = 10,
		flags = {contact = true, charge = true, protect = true, mirror = true,slicing = true},
		onTry = function(attacker, defender, move)
			if attacker:removeVolatile(move.id) then return end
			self:add('-prepare', attacker, move.name, defender)
	...
	['spectralthief'] = {
		...
		stealsBoosts = true,
		-- Boost stealing implemented in Extension
		target = "normal",
		type = "Ghost",
		zMovePower = 175,
	},
	...
	['strengthsap'] = {
		...
		onHit = function(target, source)
			...
			self:boost({atk = -1}, target, source, nil, nil, true)
		end,
		target = "normal",
		type = "Grass",
		zMoveBoost = {def = 1},
	...
	['aerialace'] = {
		...
		name = "Aerial Ace",
		pp = 20,
		flags = {contact = true, protect = true, mirror = true, distance = true, slicing = true},
		secondary = {
			chance = 10,
			self = {
				boosts = {
	...
	['aircutter'] = {
		...
		name = "Air Cutter",
		pp = 25,
		flags = {protect = true, mirror = true, slicing = true, wind = true},
		critRatio = 2,
		target = "allAdjacentFoes",
		type = "Flying"
	},
	['airslash'] = {
		...
		name = "Air Slash",
		pp = 15,
		flags = {protect = true, mirror = true, distance = true, slicing = true},
		secondary = {
			chance = 25,
			volatileStatus = 'flinch'
		},
	...
	['beatup'] = {
		...
		basePowerCallback = function(pokemon, target, move)
			if not move.multihit then
				move.multihit = pokemon.side.pokemonLeft or 6
			end
			local currentSpecies = pokemon.side.pokemon[move.multihit]
			if not currentSpecies then move.multihit = 0 return 5 end
			move.multihit -= 1
			local bp = 5 + math.floor(currentSpecies.baseStats.atk/10)
			self:debug('BP for ' .. currentSpecies.name .. ' hit: ' .. bp)
			return bp
		end,
		onModifyMove = function(move, pokemon)
			move.multihit = pokemon.side.pokemonLeft or 6
	...
	['copycat'] = {
		...
		pp = 20,
		onHit = function(pokemon)
			local noCopycat = {terastarstorm = true, assist=true, bestow=true, chatter=true, circlethrow=true, copycat=true, counter=true, covet=true, destinybond=true, detect=true, dragontail=true, endure=true, feint=true, focuspunch=true, followme=true, helpinghand=true, mefirst=true, metronome=true, mimic=true, mirrorcoat=true, mirrormove=true, naturepower=true, protect=true, ragepowder=true, roar=true, sketch=true, sleeptalk=true, snatch=true, struggle=true, switcheroo=true, thief=true, transform=true, trick=true, whirlwind=true, noxioustorque=true, combattorque=true, blazingtorque=true, wickedtorque=true, magicaltorque=true}
			local move = self:getMove(self.lastMove)
			if Not(self.lastMove) or noCopycat[self.lastMove] or move.isZ or move.isMax then return false end
	...
	['burningjealousy'] = {
		...
		secondary = {
			chance = 100,
			onHit = function(target, source, move)
				if target and target.statsRaisedThisTurn then
	...
	['electricterrain'] = {
		...
		effect = {
			...
			onStart = function(battle, source, effect)
				self:add('-terrainstart', self:getTerrainDuration())
				if effect and effect.effectType == 'Ability' then
					self:add('-fieldstart', 'move = Electric Terrain', '[from] ability = ' .. effect, '[of] ' .. source)
				else
			...
			onResidual = function()
				local dur = self:getTerrainDuration()
				local durationMessage = tostring(dur) .. " turn(s) remaining."
				self:add('-terrain', 'Electric Terrain', durationMessage, dur)
			end,
			onResidualSubOrder = 2,
			onEnd = function()
	...
	['encore'] = {
		...
		effect = {
			...
			onStart = function(target)
				...
				if Not(move) or target.volatiles['dynamax'] or Not(target.lastMove) then
					return false
				end
				if move.isMax and move.baseMove then 
					move = self:getMove(move.baseMove)
				end
				local noEncore = {encore=true, mimic=true, mirrormove=true, sketch=true, struggle=true, transform=true, noxioustorque=true, combattorque=true, blazingtorque=true, wickedtorque=true, magicaltorque=true}
				local moveIndex = indexOf(target.moves, move.id)

				if move.isZ or noEncore[move.id] or (target.moveset[moveIndex] and target.moveset[moveIndex].pp <= 0) then
					return false
				end
				self.effectData.move = move.id
	...
	['entrainment'] = {
		...
		onTryHit = function(target, source)
			...
			local bannedSourceAbilities = {flowergift=true, forecast=true, illusion=true, imposter=true, multitype=true, stancechange=true, trace=true, zenmode=true, asone=true}
	...
	['featherdance'] = {
		...
		name = "Feather Dance",
		pp = 15,
		flags = {protect = true, reflectable = true, mirror = true, dance = true},
		boosts = {
			atk = -2
		},
		target = "normal",
	...
	['grassyterrain'] = {
		...
		effect = {
			...
			onModifyPriority = function(priority, source, target, move)
				local gg = {grassyglide=true}
				if gg[move.id] then
					self:debug("Grassy Glide Priority")
					return priority + 1
				end
			end,
			onResidualOrder = 5,
			...
			onResidual = function(battle)
				...
				for _, side in pairs(battle.sides) do
					for _, pokemon in pairs(side.active) do
						if pokemon ~= null and pokemon:isGrounded() and not pokemon:isSemiInvulnerable() then
							self:debug('Pokemon is grounded, healing through Grassy Terrain.')
				...
				local dur = self:getTerrainDuration()
				local durationMessage = tostring(dur) .. " turn(s) remaining."
				self:add('-terrain', 'Grassy Terrain', durationMessage, dur)
			end,
			onEnd = function()
	...
	['gust'] = {
		...
		name = "Gust",
		pp = 35,
		flags = {protect = true, mirror = true, distance = true, wind = true},
		target = "any",
		type = "Flying"
	},
	['gyroball'] = {
	...
	['happyhour'] = {
		...
		onTryHit = function(target, source)
			...
			source.side.doublePrizeMoney = true
		end,
		target = "allySide",
		type = "Normal"
	},
	...
	['healblock'] = {
		...
		effect = {
			duration = 5,
			durationCallback = function(target, source, effect)
				if effect.id == 'psychicnoise' then
					return 2
				end
				return 5
			end,
			...
			onTryHeal = function(damage, target, source, effect)
				if effect and (effect.id == 'zpower' or self.effectData.isZ) then return damage end
				return false
			end,
	...
	['knockoff'] = {
		...
		onBasePower = function(basePower, source, target)
			local item = target:getItem()
			local disabled = item.onTakeItem and item.onTakeItem(item, target) == false
			if item and item.name and item.name ~= '' and not disabled then
				return self:chainModify(1.5)
			end
		end,
	...
	['lightscreen'] = {
		...
		effect = {
			...
			onStart = function(side)
				self:add('-sidestart', side, 'move = Light Screen', side:getSCDur('lightscreen'))
			end,
			onResidualOrder = 21,
			onResidualSubOrder = 1,
			onEnd = function(side)
	...
	['lunardance'] = {
		...
		name = "Lunar Dance",
		pp = 10,
		flags = {snatch = true, heal = true, dance = true},
		onTryHit = function(pokemon, target, move)
			if not pokemon.side:canSwitch(pokemon.position) then
				move.selfdestruct = nil
				return false
	...
	['mefirst'] = {
		...
		onTryHit = function(target, pokemon)
			local decision = self:willMove(target)
			if decision then
				local noMeFirst = {chatter=true, counter=true, covet=true, focuspunch=true, mefirst=true, metalburst=true, mirrorcoat=true, struggle=true, thief=true, noxioustorque=true, combattorque=true, blazingtorque=true, wickedtorque=true, magicaltorque=true}
	...
	['mimic'] = {
		...
		flags = {protect = true, authentic = true},
		onHit = function(target, source)
			local disallowedMoves = {terastarstorm = true, chatter=true, mimic=true, sketch=true, struggle=true, transform=true, noxioustorque=true, combattorque=true, blazingtorque=true, wickedtorque=true, magicaltorque=true}
	...
	['mistyterrain'] = {
		...
		effect = {
			...
			onResidual = function()
				local dur = self:getTerrainDuration()
				local durationMessage = tostring(dur) .. " turn(s) remaining."
				self:add('-terrain', 'Misty Terrain', durationMessage, dur)
			end,
			onResidualSubOrder = 2,
			onEnd = function(side)
	...
	['protect'] = {
		...
		effect = {
			...
			onTryHit = function(target, source, move)
				if not move.flags['protect'] then return end
				self:add('-activate', target, 'Protect')
				local lockedmove = source:getVolatile('lockedmove')
				if lockedmove then
					-- Outrage counter is reset
					if source.volatiles['lockedmove'].duration == 2 then
	...
	['reflect'] = {
		...
		effect = {
			...
			onStart = function(side)
				self:add('-sidestart', side, 'Reflect', side:getSCDur('reflect'))
			end,
			onResidualOrder = 21,
			onEnd = function(side)
	...
	['rest'] = {
		...
		onTry = function(source)
			...
			if source:hasAbility({"insomnia", "vitalspirit"}) then
				self:add('-fail', source, '[from] ability = ' + source:getAbility().name, '[of] ' + source)
			end
		end,
		onHit = function(target, source, move)
			local result = target:setStatus('slp', source, move)
	...
	['roleplay'] = {
		...
		flags = {authentic = true},
		onTryHit = function(target, source)
			local bannedAbilities = {flowergift=true, forecast=true, illusion=true, imposter=true, multitype=true, trace=true, wonderguard=true, zenmode=true, asone=true}
	...
	['sketch'] = {
		...
		flags = {authentic = true},
		onHit = function(target, source)
			local targetMove = target.lastMove
			local disallowedMoves = {chatter=true, sketch=true, struggle=true}
			if source.transformed or (not target.lastMove or target.lastMove == '') or disallowedMoves[target.lastMove] or indexOf(source.moves, target.lastMove) or targetMove.isZ or targetMove.isMax  or targetMove.noSketch then return false end
			...
			source.moves[moveslot] = toId(move.name)
			source.set.moves[moveslot] = sketchedMove
	...
	['sleeptalk'] = {
		...
		onHit = function(pokemon)
			local moves = {}
			local NoSleepTalk = {assist=true, belch=true, bide=true, chatter=true, copycat=true, focuspunch=true, mefirst=true, metronome=true, mimic=true, mirrormove=true, naturepower=true, sketch=true, sleeptalk=true, uproar=true, noxioustorque=true, combattorque=true, blazingtorque=true, wickedtorque=true, magicaltorque=true}
	...
	['spikes'] = {
		...
		effect = {
			...
			onSwitchIn = function(pokemon)
				...
				if pokemon:hasAbility('tangledfeet') then return end
	...
	['teleport'] = {
		...
		priority = -6,
		onTryHit = function(target, source, effect)
			if (source.volatiles['partiallytrapped'] or source.trapped or source.maybeTrapped) and self.p2.name == '#Wild' then return false end
			if self.p2.name == '#Wild' and not self.isRaid then
				self:add('-flee', source)
				self:win(self.p1)
				return 0
			end
			return source.side:canSwitch(source.position)
		end,
		selfSwitch = true,
		target = "self",
		type = "Psychic"
	...
	['thief'] = {
		...
		onAfterHit = function(target, source, move)
			if not Not(source.item) or source.volatiles['gem'] then return end
			local yourItem = target:takeItem(source)
			if Not(yourItem) then return end
			if Not(source:setItem(yourItem)) then
				target.item = yourItem.id -- bypass setItem so we don't break choicelock or anything
			...
			self:add('-enditem', target, yourItem, '[silent]', '[from] move = Thief', '[of] ' .. source)
	...
	['toxicspikes'] = {
		...
		effect = {
			...
			onSwitchIn = function(pokemon)
				...
				if not pokemon:isGrounded() then unaffected = true end
				if item.id == 'heavydutyboots' then unaffected = true end
				if pokemon:hasAbility('tangledfeet') then unaffected = true end
				if not pokemon:runImmunity('Poison') then unaffected = true end
				if pokemon:hasType('Poison') then
					self:add('-sideend', pokemon.side, 'move = Toxic Spikes', '[of] ' .. pokemon)
					pokemon.side:removeSideCondition('toxicspikes')
				elseif self.effectData.layers >= 2 and not unaffected then
	...
	['poltergeist'] = {
		...
		onTryHit = function(target, source, move) 
			self:add('-activate', target, 'move: Poltergeist', target:getItem().name)
		end,
		target = "normal",
		type = "Ghost",
	},
	...
	['triplekick'] = {
		...
		basePowerCallback = function(pokemon)
			...
			return 20 * pokemon.volatiles['triplekick'].hit
		end,
		category = "Physical",
		id = "triplekick",
		name = "Triple Kick",
	...
	['whirlwind'] = {
		...
		pp = 20,
		priority = -6,
		flags = {reflectable = true, mirror = true, authentic = true, wind = true},
		forceSwitch = true,
		target = "normal",
		type = "Normal"
	},
